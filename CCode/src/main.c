...
static void print_usage(const char *prog) {
    fprintf(stderr, "Help: %s -in <dataset> -s <solution> -out <results> -nj <workers> [options]\n", prog);
    fprintf(stderr, "Options:\n");
    fprintf(stderr, "  -k, --num-clusters INT   Number of clusters to form (default: %d)\n", DEFAULT_NUM_CLUSTERS);
    fprintf(stderr, "  -ml, --max-load INT      Max time to complete a route per cluster (default: %d)\n", DEFAULT_MAX_LOAD_SECONDS);
    fprintf(stderr, "  -p, --penalization {0,1} Active penalization in the objective function (default: 1)\n");
...
static int parse_args(int argc, char **argv, ProgramArgs *args) {
    static struct option long_opts[] = {
        ...
        {"max-load",     required_argument, 0, 'm'},
...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < args.num_clusters; ++i) {
